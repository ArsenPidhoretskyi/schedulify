...
from schedulify.apps.accounts.exceptions import (
...
PASSWORD_SERVICE_PATH = "schedulify.apps.accounts.services.password.PasswordService"


def test_change_password(user_account):
    old_password = "old_password_1234"
    new_password = "new_password_5678"
...
def test_reset_password_signature_failure(mocker, settings, exception):
    new_password = "new_password_1234"
    reset_password_signature = "reset_password_signature"
    mocker.patch("django.core.signing.TimestampSigner.unsign", side_effect=exception("Some message"))
    settings.schedulify_RESET_PASSWORD_EXPIRATION_DELTA = 42
    mocked_change_password = mocker.patch(f"{PASSWORD_SERVICE_PATH}.change_password")
